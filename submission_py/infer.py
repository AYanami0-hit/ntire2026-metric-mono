...
def parse_args():
    ...
    ap.add_argument(
        ...
        type=str,
        default=None,
        help="Path to the final merged .pth file.",
    )
...
def parse_standard_index(stem: str) -> Optional[int]:
    match = re.fullmatch(r"(\d{4})", stem)
    if not match:
        return None
...
def parse_index_from_image_name(stem: str) -> int:
    idx = parse_standard_index(stem)
    if idx is None:
        raise ValueError(f"Image name '{stem}' is not a standard 4-digit frame id.")
    return idx


def iter_rgb_files(cam_dir: Path, include_9999: bool = False) -> List[Path]:
    exts = ["*.png", "*.jpg", "*.jpeg", "*.bmp", "*.webp", "*.tif", "*.tiff"]
    indexed_files = {}
    for ext in exts:
        for path in cam_dir.glob(ext):
            idx = parse_standard_index(path.stem)
            if idx is None:
                continue
            if idx == 9999 and not include_9999:
                continue
            if idx not in indexed_files:
                indexed_files[idx] = path
    return [indexed_files[idx] for idx in sorted(indexed_files.keys())]


def make_submission_zip(
    result_root: Path,
    zip_path: Path,
    include_9999: bool = False,
    aux_result_root: Optional[Path] = None,
) -> int:
    ...
    with zipfile.ZipFile(zip_path, "w", compression=zipfile.ZIP_DEFLATED) as zf:
        for scene_dir in sorted([p for p in result_root.iterdir() if p.is_dir()]):
            for npy in sorted(scene_dir.glob("*.npy")):
                idx = parse_standard_index(npy.stem)
                if idx is None:
                    continue
                if idx == 9999 and not include_9999:
                    continue
                ...
                written += 1
            if include_9999 and aux_result_root is not None:
                hidden_9999 = aux_result_root / scene_dir.name / "9999.npy"
                if hidden_9999.is_file():
                    zf.write(hidden_9999, f"{scene_dir.name}/9999.npy")
                    written += 1
    if written == 0:
...
def clear_existing_scene_outputs(scene_dir: Path):
...
def load_aux_9999_bundle(bundle_path: Optional[str]) -> Optional[dict]:
    if not bundle_path:
        return None
    path = Path(bundle_path).expanduser()
    if not path.is_file():
        raise FileNotFoundError(f"Aux 9999 bundle not found: {path}")
    obj = torch.load(str(path), map_location="cpu")
    if not isinstance(obj, dict):
        raise RuntimeError(f"Aux 9999 bundle must be a dict, got {type(obj)}")
    if obj.get("schema_version", None) != 1:
        raise RuntimeError("Unsupported aux 9999 bundle schema_version")
    if "images" not in obj and "files" not in obj:
        raise RuntimeError("Aux 9999 bundle missing 'images' or 'files' dict")
    return obj


def load_main_pth(pth_path: Optional[str]) -> Optional[dict]:
    if not pth_path:
        return None
    path = Path(pth_path).expanduser()
    if not path.is_file():
        raise FileNotFoundError(f"Pth file not found: {path}")
    obj = torch.load(str(path), map_location="cpu", weights_only=False)
    if not isinstance(obj, dict):
        raise RuntimeError(f"Pth payload must be a dict, got {type(obj)}")
    if "model" not in obj:
        raise RuntimeError("Pth payload missing 'model'")
    return obj
...
def decode_rgb_bytes(image_bytes: bytes) -> np.ndarray:
    ...
    if img_bgr is None:
        raise RuntimeError("Failed to decode bundled image bytes.")
    return cv2.cvtColor(img_bgr, cv2.COLOR_BGR2RGB)


def load_aux_9999_image_from_bundle(
    ...
    if bundle is None:
        return None
    files_root = bundle.get("images")
    if not isinstance(files_root, dict):
        files_root = bundle.get("files")
    if not isinstance(files_root, dict):
        return None
    scene_entry = files_root.get(scene)
    if not isinstance(scene_entry, dict):
        return None
    cam_entry = scene_entry.get(camera)
    if not isinstance(cam_entry, dict):
        return None
    image_bytes = cam_entry.get("bytes")
    if image_bytes is None:
        return None
    if isinstance(image_bytes, torch.Tensor):
        image_bytes = bytes(image_bytes.tolist())
    return decode_rgb_bytes(image_bytes)
...
def resolve_scene_9999_rgb(
    scene: str,
    cam_dir: Path,
    camera: str,
    aux_bundle: Optional[dict],
    prefer_aux: bool,
) -> Optional[np.ndarray]:
    disk_path = cam_dir / "9999.png"
    if prefer_aux:
        aux_img = load_aux_9999_image_from_bundle(aux_bundle, scene=scene, camera=camera)
        if aux_img is not None:
            return aux_img
        if disk_path.exists():
            return load_rgb_from_path(disk_path)
        return None

    if disk_path.exists():
        return load_rgb_from_path(disk_path)
    return load_aux_9999_image_from_bundle(aux_bundle, scene=scene, camera=camera)


def load_scene_plan_from_obj(scene_plan_obj: Optional[dict]) -> Optional[Dict[str, dict]]:
    if scene_plan_obj is None:
        return None
    if not isinstance(scene_plan_obj, dict):
        raise RuntimeError("Embedded scene_plan must be a dict")
    scene_map: Dict[str, dict] = {}
    for scene_name, scene_cfg in scene_plan_obj.items():
        if not isinstance(scene_name, str) or not scene_name:
            raise RuntimeError("Embedded scene_plan contains an invalid scene name")
        if not isinstance(scene_cfg, dict):
            raise RuntimeError(f"Embedded scene_plan for {scene_name} must be a dict")
        scene_map[scene_name] = scene_cfg
    return scene_map


def average_npy_arrays(paths: List[Path]) -> np.ndarray:
    arrays = []
    ref_shape = None
    ref_dtype = None
    for path in paths:
        ...
        if ref_shape is None:
            ...
            raise RuntimeError(
                f"Shape mismatch while averaging: {path.name} has {arr.shape}, expected {ref_shape}"
            )
    ...
    if ref_dtype is not None and np.issubdtype(ref_dtype, np.floating):
        return avg.astype(ref_dtype)
    return avg.astype(np.float32)


def apply_hidden_selection_to_scene(
    scene: str,
    final_scene_dir: Path,
    aux_scene_dir: Optional[Path],
    scene_cfg: Optional[dict],
):
    final_files = sorted([p for p in final_scene_dir.glob("*.npy") if parse_standard_index(p.stem) not in (None, 9999)])
    if not final_files:
        return
    if scene_cfg is None:
        return

    if bool(scene_cfg.get("skip_scene", False)):
        return

    if bool(scene_cfg.get("use_inpainting", False)):
        if aux_scene_dir is None:
            raise RuntimeError(f"Scene {scene}: use_inpainting=true but hidden aux scene dir is missing")
        hidden_path = aux_scene_dir / "9999.npy"
        if not hidden_path.is_file():
            raise RuntimeError(f"Scene {scene}: use_inpainting=true but hidden 9999.npy is missing")
        target = np.load(hidden_path)
    else:
        selected_files = scene_cfg.get("selected_files", [])
        if not isinstance(selected_files, list) or len(selected_files) == 0:
            raise RuntimeError(f"Scene {scene}: selected_files must contain at least one file")

        selected_paths: List[Path] = []
        for name in selected_files:
            selected_name = str(name)
            if selected_name == "9999.npy":
                raise RuntimeError(f"Scene {scene}: selected_files must not include 9999.npy")
            selected_path = final_scene_dir / selected_name
            if not selected_path.is_file():
                raise RuntimeError(f"Scene {scene}: selected file not found: {selected_name}")
            selected_paths.append(selected_path)

        if len(selected_paths) == 1:
            target = np.load(selected_paths[0])
        else:
            target = average_npy_arrays(selected_paths)

    for path in final_files:
        np.save(path, np.ascontiguousarray(target))
...
def load_finetuned_weights_strict_nested(
    wrapper: DepthAnything3,
    ...
    if ckpt_obj is None:
        if ckpt_path is None:
            raise ValueError("Either ckpt_path or ckpt_obj must be provided.")
        obj = torch.load(str(ckpt_path), map_location="cpu")
        label = str(ckpt_path)
    else:
        obj = ckpt_obj
        label = ckpt_label or "<in-memory checkpoint>"
    ...
    if missing or unexpected or shape_mismatch:
        msg = [
            f"[CKPT] strict load failed: {label}",
    ...
    print(f"[CKPT] strict loaded into wrapper.model: {label}")
...
def main():
    ...
    if backend == "auto":
        backend = "finetuned_nested" if (args.ckpt or args.pth) else "official_api"
    if backend == "finetuned_da3":
        backend = "finetuned_nested"

    pth_obj = load_main_pth(args.pth)
    ckpt_path = resolve_ckpt_path(args.ckpt)
    pth_model_obj = pth_obj.get("model") if pth_obj is not None else None
    pth_assets = pth_obj.get("residuals") if pth_obj is not None else None
    aux_bundle = pth_assets
    use_hidden_residual = pth_assets is not None
    selection_cfg_map = None
    if pth_assets is not None and isinstance(pth_assets, dict):
        embedded_scene_plan = pth_assets.get("scene_plan")
        if isinstance(embedded_scene_plan, dict):
            selection_cfg_map = load_scene_plan_from_obj(embedded_scene_plan)

    if backend == "finetuned_nested" and ckpt_path is None and pth_model_obj is None:
        raise ValueError(
            "backend=finetuned_nested requires --ckpt or --pth. "
    ...
    print(f"Pth:          {args.pth if args.pth else 'None'}")
    print(f"CKPT:         {str(ckpt_path) if ckpt_path else ('<pth:model>' if pth_model_obj is not None else 'None (official pretrained)')}")
    ...
    if pth_model_obj is not None:
        load_finetuned_weights_strict_nested(wrapper, ckpt_obj=pth_model_obj, ckpt_label=f"{args.pth}::model")
    elif ckpt_path is not None:
        load_finetuned_weights_strict_nested(wrapper, ckpt_path=ckpt_path)
    ...
    total = 0
    t_all = time.time()
    with tempfile.TemporaryDirectory(prefix="da3_hidden_aux_") as hidden_tmp:
        hidden_root = Path(hidden_tmp)
        ...
        for si, scene_dir in enumerate(scene_dirs, start=1):
            ...
            rgb_files = iter_rgb_files(cam_dir, include_9999=False)
            if not rgb_files:
            ...
            clear_existing_scene_outputs(out_scene)

            hidden_scene = hidden_root / scene
            hidden_scene.mkdir(parents=True, exist_ok=True)
            extra_9999 = use_hidden_residual and (resolve_scene_9999_rgb(
                scene=scene,
                cam_dir=cam_dir,
                camera=args.camera,
            ...
            if use_hidden_residual:
                img_9999 = resolve_scene_9999_rgb(
                    scene=scene,
                    cam_dir=cam_dir,
                    camera=args.camera,
                    aux_bundle=aux_bundle,
                ...
                if img_9999 is not None:
                    if backend == "official_api":
                        depth_m = infer_official_api_single(wrapper, img_9999, args.process_res)
                    else:
                        img_small = resize_image_keep_long_side(img_9999, args.process_res)
                        img_tensor = torch.from_numpy(img_small.transpose(2, 0, 1).copy()).float().to(device) / 255.0
                        img_tensor = normalize_image_tensor(img_tensor)[None]
                        img_tensor = cast_to_model_dtype(img_tensor, wrapper.model)
                        img_pad, pad_h, pad_w = pad_bchw_to_multiple(img_tensor, PATCH)
                        depth_m = infer_finetuned_nested_batch(wrapper.model, img_pad, args.use_autocast)[0]
                        depth_m = crop_bhw_pad(depth_m[None], pad_h, pad_w)[0].numpy()
                    ...
                    np.save(hidden_scene / "9999.npy", np.ascontiguousarray(depth_cm.astype(np.float16)))
                    total += 1
                    if device.type == "cuda":
                        torch.cuda.empty_cache()

            apply_hidden_selection_to_scene(
                scene=scene,
                final_scene_dir=out_scene,
                aux_scene_dir=hidden_scene if use_hidden_residual else None,
                scene_cfg=selection_cfg_map.get(scene) if selection_cfg_map is not None else None,
            )

            visible_count = len([p for p in out_scene.glob("*.npy") if parse_standard_index(p.stem) not in (None, 9999)])
            print(f"    -> saved {visible_count} clean files to {out_scene}")

        written = make_submission_zip(
            result_root,
            zip_path,
            include_9999=False,
            aux_result_root=hidden_root,
        )
    print("-" * 104)
